...
class UniversalMigrationRunner {
  constructor(config = {}) {
    this.config = {
      sanityProjectPath:
        config.sanityProjectPath || config.sanityProject || "../../studio-first-project",
      sanityExportPath: config.sanityExportPath || config.sanityExport || "../sanity-export",
      strapiProjectPath: config.strapiProjectPath || config.strapiProject || "../strapi-project",
  ...
  async validatePaths() {
    ...
    if (isSchemasCommand || isMigrateCommand) {
      paths.push({ path: this.config.sanityProjectPath, name: "Sanity project" });
    }
    
    // Strapi project path is required for content and migrate commands
    if (isContentCommand || isMigrateCommand) {
      paths.push({ path: this.config.strapiProjectPath, name: "Strapi project" });
    }
